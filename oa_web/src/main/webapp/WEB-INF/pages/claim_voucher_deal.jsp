...
<%@ page import="com.hjf.oa.global.Constant" %>
                    ...
                    <table id="message-table" class="table admin-form theme-warning tc-checkbox-1">
                        ...
                        <c:forEach items="${list}" var="cv">
                            <tr class="message-unread">
                                ...
                                <td>
                                    <c:if test="${cv.status == Constant.CLAIM_STATUS_CREATED || cv.status == Constant.CLAIM_STATUS_BACK}">
                                        <a href="/claim_voucher/to_update?id=${cv.id}">修改</a>
                                        <a href="/claim_voucher/submit?id=${cv.id}">提交</a>
                                    </c:if>
                                    <c:if test="${cv.status == Constant.CLAIM_STATUS_SUBMIT || cv.status == Constant.CLAIM_STATUS_RECHECK}">
                                        <a href="/claim_voucher/to_check?id=${cv.id}">审核</a>
                                    </c:if>
                                    <c:if test="${cv.status == Constant.CLAIM_STATUS_APPROVED}">
